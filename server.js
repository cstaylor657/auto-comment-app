...
app.post('/webhook', async (req, res) => {
  ...
  for (const change of entry.changes) {
    if (change.field === 'messages') {
      ...
      // Find account
      db.get('SELECT access_token FROM accounts WHERE account_id = ? AND enabled = 1', [igUserId], async (err, row) => {
        if (row) {
          await handleComment(commentId, value.message, igUserId, row.access_token);
        }
      });
    }
  }
